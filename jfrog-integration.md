...
1. dump data from single node cluster(127.0.0.1)
    ...
    - [TODO] dump from PostgreSQL, then dump data to YB Cluster
...
1. Login to the UI
    ...
    - default login (admin/password)
      - will need to update the default password
...
3. Download the uploaded data from artifactory
...
### [PostgreSQL] Schema in postgresql(postgresql@11 on OSX with `brew install`)
...
### Apply some missing, not applied queries.

- checking the table mis match in above then,  add the queries in `artifactory-storage-db-7.35.2.jar`, the path will be `/artifactory/app/artifactory/tomcat/webapps/artifactory/WEB-INF/lib/`
...
Logs for YB Single Node can be found in [gist](https://gist.github.com/tichimura/f5d96810fa368ad0a9c399cb593d7c48)
